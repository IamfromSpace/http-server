...
function HttpServer(options) {
  ...
  if (options.cors) {
    ...
    // If the origins option is present, we conditionally set the allowed origin.
    // Otherwise, we allow all origins.
    if (options.origins) {
      before.push(function (req, res) {
        // If we've specified a pattern of allowed origins, we test for a match
        // and then attach the current origin if it matches.  Otherwise we leave
        // off the header, so the browser will deny the request.
        if (new RegExp(options.origins).test(req.headers.origin)) {
